...
def t_test(xdata, ydata, equal_var=True, alpha=0.05):
	x = dropnan(xdata)
	if is_iterable(ydata):
		y = dropnan(ydata)
		t, p = st.ttest_ind(x, y, equal_var=equal_var)
		print "t-test"
	else:
	...
	if not is_iterable(data):
		...
		for i, d in enumerate(data):
			if len(d) == 0:
				...
				print "{:<10}".format(count) + "{:<10}".format(mean) + "{:<10}".format(std) + "{:<10}".format(vmax) + "{:<10}".format(median) + "{:<10}".format(vmin) + "{:<10}".format(groups[i])
		print ""
		pass

def graphHisto(data, bins=20, name='Data', color='green', boxplot=True):
	x = dropnan(data)
...
def graphScatter(xData, yData, xName='x', yName='y', fit=True, pointStyle='k.', lineStyle='r-'):
	x, y = dropnan_intersect(xData, yData)
...
def graphBoxplot(values, groups=[], valueName='Values', categories='Categories', probplot=True):
...
#					groups = groups[:i] + groups[i + 1:]
		...
		if probplot == True:
			...
			for i, g in enumerate(prob):
				plt.plot(g[0][0], g[0][1], marker='^', color=color(i), label=groups[i])
				plt.plot(g[0][0], g[1][0] * g[0][0] + g[1][1], linestyle='-', color=color(i))
			plt.legend(loc='best')
...
def analyze(xdata, ydata=[], groups=[], xName='x', yName='y', alpha=0.05, categories='Categories'):
	if any(is_iterable(x) for x in xdata):
		graphBoxplot(xdata, groups, xName, categories)
		groupStats(xdata, groups)
